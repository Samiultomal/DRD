...
import { fetchPostCategories, createSingleCategory, getLocationDetails } from '../../../api/SingleCategoryApi';
...
function AddSingleCategory() {
  ...
  useEffect(() => {
    ...
    const fetchLocationDetails = async () => {
      try {
        ...
        setRegionName(locationData.region_name);
      } catch (err) {
  ...
  const handleCategoryClick = async (categoryId) => {
    ...
    try {
      ...
      navigate(`/categories/${createdCategory.id}/posts/add`);
    } catch (err) {
  ...
  const breadcrumbsItems = [
    ...
    { label: `Location: ${regionName || 'Loading...'}`, path: `/add-single-category/${locationId}` },
  ];
